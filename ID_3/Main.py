...
def entropy(column_name):
    ...
    uniqueValues, occurenceCount = np.unique(column_name, return_counts=True)    # array of unique values, number of occurence of each unique value
    entropy = 0

    # implementation of the entropy formula
    for i in range(len(uniqueValues)):
        entropy += (-occurenceCount[i] / np.sum(occurenceCount)) * np.log2(occurenceCount[i] / np.sum(occurenceCount))
...
def InfoGain(data, attribute_name):
    ...
    total_entropy = entropy(data["name"])   # Calculate the entropy of the total dataset
    values, counts = np.unique(data[attribute_name], return_counts=True)        # Calculate the values and the corresponding counts for the split attribute

    # Calculate the weighted entropy
    weighted_entropy = 0
    for i in range(len(values)):
        weighted_entropy += (counts[i] / np.sum(counts)) * entropy(data.where(data[attribute_name] == values[i]).dropna()["name"])
...
def ID3(data, originaldata, features, parent_node_class=None):
    ...
    # If none of the above holds true, grow the tree!
        ...
        for value in np.unique(data[best_feature]):
            ...
            subtree = ID3(sub_data, dataset, features, parent_node_class)
